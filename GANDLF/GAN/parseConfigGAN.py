...
from GANDLF.data.post_process import (
    postprocessing_after_reverse_one_hot_encoding,
)
...
def parseConfigGAN(config_file_path, version_check_flag=True):
    ...
    if not ("step_size" in params["scheduler_g"]):
        ...
        print(
            ...
            params["scheduler_g"]["step_size"],
        )
    if not ("step_size" in params["scheduler_d"]):
        params["scheduler_d"]["step_size"] = params["learning_rate_d"] / 5.0
        print(
            "WARNING: Setting default discriminator scheduler step_size to:",
            params["scheduler_d"]["step_size"],
        )
    ...
    if isinstance(params["optimizer_g"], str):
        temp_dict = {}
        temp_dict["type"] = params["optimizer_g"]
        params["optimizer_g"] = temp_dict
    if isinstance(params["optimizer_d"], str):
        temp_dict = {}
