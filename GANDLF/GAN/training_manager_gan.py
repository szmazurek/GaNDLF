...
def TrainingManagerGAN(
    dataframe, outputDir, parameters, device, resume, reset
):
    """
    ...
    for trainAndVal_index, testing_index in kf_testing.split(subjectIDs_full):
        ...
        if noTestingData:
            ...
            if (
                (not os.path.exists(currentTestingDataPickle))
                or reset
                or resume
            ):
        ...
        for train_index, val_index in kf_validation.split(
            ...
            if (
                (not os.path.exists(currentTrainingDataPickle))
                or reset
                or resume
            ):
                trainingData.to_pickle(currentTrainingDataPickle)
                trainingData.to_csv(
                    currentTrainingDataPickle.replace(".pkl", ".csv"),
                    index=False,
                )
            else:
            ...
            if (
                ...
                validationData.to_csv(
                    currentValidationDataPickle.replace(".pkl", ".csv"),
                    index=False,
                )
            else:
